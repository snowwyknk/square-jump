...
# Импорт модулей
from src.enums import GameState, ObstacleType, BonusType, EffectType, QuestType
from src.localization import Localization
from src.player import Player
from src.obstacles import Obstacle, Amethyst, Bonus
from src.particles import ParticleSystem
from src.shop import ShopItem
from src.animations import Animation, Animator
from src.ui import Button, ProgressBar, UIManager
from src.graphics_engine import BackgroundGenerator, GlowEffect, ParticleExplosion, SpeedLines
from src.texture_cache import TextureCache

# Импорт новых улучшенных модулей
from src.analytics import Analytics
from src.ui_statistics import StatisticsPanel, AchievementBadge, PerformanceGraph, LevelProgressionChart, StatisticsScreen
from src.themes import ThemeManager, ThemeType, Color
from src.enhancements import EnhancedPlayer, EnhancedObstacle, EnhancedParticleSystem, PerformanceMonitor
...
class Game:
    def __init__(self):
        ...
        self.BASE_SCREEN_WIDTH = 1366
        self.BASE_SCREEN_HEIGHT = 768
        self.BLACK = (0, 0, 0)
        ...
        self.max_fps = 60  # FPS cap - оптимальный баланс
        self.frame_time = 1000.0 / self.max_fps  # Время кадра в мс
        self.frame_times = []  # История времён кадров для мониторинга производительности
        ...
        # ОПТИМИЗАЦИЯ: Снизили лимит частиц для лучшей производительности
        self.max_particles_limit = 200
        self.particle_system = ParticleSystem(self.max_particles_limit)
        self.min_particles_limit = 100  # Минимальное количество частиц
        
        # Кэш для текстур и поверхностей
        self.texture_cache = TextureCache(max_size=500)
        ...
        # Кэш для поверхностей препятствий
        self.obstacle_surface_cache = {}
        
        # Загрузка анимаций (пример)
        try:
            self.load_animations()
        except Exception as e:
            logging.error(f"Error loading animations: {e}")
        ...
        # Кэшированные константы для производительности
        self.ground_y = int(self.SCREEN_HEIGHT * 0.833)
        self.ground_buffer = 8
        self.death_y = int(self.SCREEN_HEIGHT * 1.17)
        ...
        self._cached_upgrades = None
        self._cached_skins = None
        ...
        # UI элементы удалены - используется только pygame-menu
        
        # 🎨 СИСТЕМА ГРАФИКИ
        self.bg_generator = BackgroundGenerator(self.SCREEN_WIDTH, self.SCREEN_HEIGHT)
        self.explosions = []
        self.speed_lines_list = []
        ...
        self.chest_spin_items = []
        self.chest_spin_index = 0
        self.chest_spin_running = False
        self.chest_spin_timer = 0
        self.chest_spin_duration = 0
        self.chest_selected_index = None
        self.chest_drops = []
        ...
        # ✨ НОВЫЕ СИСТЕМЫ УЛУЧШЕНИЙ
        # Аналитика
        self.analytics = Analytics(data_dir="data")
        ...
        # Статистика UI
        self.stats_screen = StatisticsScreen(self.SCREEN_WIDTH, self.SCREEN_HEIGHT, self.analytics)
        ...
        # Флаги управления нажатиями клавиш
        self.tab_pressed = False
        self.f1_pressed = False
        self.number_key_pressed = {i: False for i in range(1, 8)}
        ...
        # Режим генерации уровней
        self.enable_random_generation = False
    ...
    def load_animations(self) -> None:
        """Загрузка анимаций для игры"""
        try:
            # Пример анимации для игрока (пока простые прямоугольники)
            # В будущем можно загрузить спрайты из файлов
            idle_frames = []
            for i in range(4):
                frame = pygame.Surface((self.player.size, self.player.size), pygame.SRCALPHA)
                # Создаем простой анимированный кадр
                color = (100 + i * 30, 150, 200 + i * 10)
                pygame.draw.rect(frame, color, (0, 0, self.player.size, self.player.size), border_radius=10)
                idle_frames.append(frame)
            
            idle_animation = Animation(idle_frames, frame_duration=15, loop=True)
            self.animator.add_animation("player_idle", idle_animation)
            
            # Анимация прыжка
            jump_frames = []
            for i in range(3):
                frame = pygame.Surface((self.player.size, self.player.size), pygame.SRCALPHA)
                scale = 1.0 + 0.1 * math.sin(i * math.pi / 2)
                size = int(self.player.size * scale)
                color = (200, 100 + i * 50, 100)
                pygame.draw.rect(frame, color, (0, 0, size, size), border_radius=10)
                jump_frames.append(frame)
            
            jump_animation = Animation(jump_frames, frame_duration=8, loop=False)
            self.animator.add_animation("player_jump", jump_animation)
            
            logging.info("Animations loaded successfully")
        except Exception as e:
            logging.error(f"Error loading animations: {e}")
    ...
    def create_gradient_menu_bg(self):
        """Создает красивый анимированный фон меню с градиентом"""
        ...
        # УЛУЧШЕНИЕ: Более красивые начальные и конечные цвета
        start_color = self.hsv_to_rgb(base_hue, 0.7, 0.25)
        middle_color = self.hsv_to_rgb((base_hue + 30) % 360, 0.6, 0.35)
        end_color = self.hsv_to_rgb((base_hue + 60) % 360, 0.8, 0.5)
        
        # Создаем двойной вертикальный градиент для большей глубины
        for y in range(self.SCREEN_HEIGHT):
            t = y / self.SCREEN_HEIGHT
            if t < 0.5:
                # Верхняя половина: от start к middle
                t_local = t * 2
                r = self.lerp(start_color[0], middle_color[0], t_local)
                g = self.lerp(start_color[1], middle_color[1], t_local)
                b = self.lerp(start_color[2], middle_color[2], t_local)
            else:
                # Нижняя половина: от middle к end
                t_local = (t - 0.5) * 2
                r = self.lerp(middle_color[0], end_color[0], t_local)
                g = self.lerp(middle_color[1], end_color[1], t_local)
                b = self.lerp(middle_color[2], end_color[2], t_local)
            pygame.draw.line(surf, (int(r), int(g), int(b)), (0, y), (self.SCREEN_WIDTH, y))
        
        # УЛУЧШЕНИЕ: Больше звёзд с разной яркостью
        for _ in range(80):
            x = random.randint(0, self.SCREEN_WIDTH)
            y = random.randint(0, self.SCREEN_HEIGHT)
            size = random.randint(1, 3)
            brightness = random.randint(100, 255)
            color_val = min(255, brightness + 50)
            pygame.draw.circle(surf, (brightness, brightness, color_val), (x, y), size)
        
        # УЛУЧШЕНИЕ: Добавляем тонкие линии для улучшения текстуры
        for _ in range(5):
            x1 = random.randint(0, self.SCREEN_WIDTH)
            y1 = random.randint(0, self.SCREEN_HEIGHT)
            x2 = x1 + random.randint(-200, 200)
            y2 = y1 + random.randint(-200, 200)
            pygame.draw.line(surf, (100, 150, 200), (x1, y1), (x2, y2), 1)
    ...
    def update_menu_theme(self):
        """Обновление темы меню с улучшенным стилем"""
        theme = pygame_menu.themes.THEME_DARK.copy()
        
        # Настраиваем тему для более красивого вида
        theme.title_background_color = (20, 20, 50, 200)
        theme.title_font_color = (255, 215, 0)  # Золото
        theme.widget_font_color = (255, 255, 255)
        theme.widget_font_selected_color = (255, 215, 0)
        ...
        theme.widget_border_color = (100, 100, 200)
        theme.widget_box_background_color = (30, 30, 60, 150)
    ...
    def create_menus(self):
        ...
        try:
            ...
            # Главное меню с чистым, современным интерфейсом
            ...
            # Подготовка меню подтверждения выхода (будет показано при ESC на главном меню)
            self.exit_confirm_menu = pygame_menu.Menu(
                self.localization.get('confirm_exit_title'),
                self.SCREEN_WIDTH,
                self.SCREEN_HEIGHT,
                theme=theme
            )
            self.exit_confirm_menu.add.label(self.localization.get('confirm_exit_text'), font_color=self.WHITE, font_size=24)
            self.exit_confirm_menu.add.button(self.localization.get('yes'), self.exit_game)
            self.exit_confirm_menu.add.button(self.localization.get('no'), self.return_to_main_menu)

            
            # GAMEPLAY SECTION
            self.main_menu.add.label("GAMEPLAY", font_color=self.GOLD, font_size=24)
            ...
            # SHOP & CHARACTER SECTION
            self.main_menu.add.label("SHOP & CHARACTER", font_color=self.GOLD, font_size=24)
            ...
            # GAMEPLAY INFO & SETTINGS
            self.main_menu.add.label("GAMEPLAY INFO & SETTINGS", font_color=self.GOLD, font_size=24)
            ...
            # Меню "Прочее" - используем тему как и для других меню
    ...
    def start_game_from_menu(self):
        """Запуск игры из меню"""
        logging.info("start_game_from_menu called")
        try:
            if self.menu and self.menu.is_enabled():
                logging.debug("Disabling menu in start_game_from_menu")
                self.menu.disable()
        except Exception as e:
            logging.warning(f"Error disabling menu in start_game_from_menu: {e}")
    ...
    def update_achievements_menu(self):
        ...
        if unlocked:
            unlocked_names = [self.localization.get(f'achievement_{name}', name) for name in unlocked]
            self.achievements_menu.add.label("✓ " + ", ".join(unlocked_names), font_color=self.CYAN, font_size=18)

        # Список доступных достижений
        self.achievements_menu.add.label("─" * 40, font_color=self.WHITE)
        for key in self.achievements:
            status = "✓" if self.achievements[key] else "…"
            # Получаем локализованное название достижения
            achievement_name = self.localization.get(f'achievement_{key}', key.replace('_', ' ').title())
            achievement_desc = self.localization.get(f'achievement_{key}_desc', '')
            
            text = f"{status} {achievement_name}"
            color = self.GREEN if self.achievements[key] else self.WHITE
            
            self.achievements_menu.add.label(text, font_color=color, font_size=22)
            if achievement_desc:
                self.achievements_menu.add.label(achievement_desc, font_color=(200, 200, 200), font_size=16)
    ...
    def change_resolution(self, value, index):
        ...
        resolutions = [(1366, 768), (1024, 768), (1280, 720), (800, 600)]
        ...
        if not self.fullscreen:
            ...
            if hasattr(self, 'game_bg') and self.game_bg:
    ...
    def buy_shop_item_from_menu(self, item):
        ...
        if success:
            ...
            success_menu.add.label(f'{self.shop_items[self.selected_shop_item].get_name()} {self.localization.get("purchase_success").lower()}', font_color=self.GREEN)
    ...
    def start_preset_level(self, index):
        """Запуск пресет-уровня с ленивой генерацией"""
        logging.info(f"start_preset_level called with index={index}")
        if 0 <= index < len(self.preset_levels):
            level_data = self.preset_levels[index]
            
            # ОПТИМИЗАЦИЯ: Ленивая генерация для еще не сгенерированных уровней
            if level_data.get("_lazy_generated") is False:
                logging.info(f"Lazy generating level {index}...")
                level_data = self.generate_random_preset_level(level_data.get("_level_num", index))
                self.preset_levels[index] = level_data  # Кэшируем сгенерированный уровень
                logging.info(f"Level {index} generated")
 ...
 # ========================================================================
    ...
    def calculate_level_progress(self):
        ...
        return int(self.distance_traveled / 15)  # Для случайной генерации не ограничиваем
    ...
    def compute_level_end_x(self, level_data):
        ...
        # Рассчитываем на основе объектов
        calculated_end = max(max_obj + 500, max_ame + 500, 2000)
        
        # Используем level_end_x из файла, если он больше вычисленного
        json_level_end_x = level_data.get("level_end_x", calculated_end)
        
        # Возвращаем максимум из обоих значений
        return max(calculated_end, json_level_end_x)
    ...
    def award_exp_on_level_complete(self, level_name):
        if not self.player or not hasattr(self.player, 'skin_level'):
            ...
            return
        # Начисляем опыт и отмечаем уровень как пройденный только один раз
        if level_name in self.completed_levels:
            logging.info(f"EXP for level {level_name} already awarded; skipping")
            return

        # Отмечаем как пройденный и сохраняем прогресс
    ...
    def generate_preset_levels(self):
        """ОПТИМИЗАЦИЯ: Генерируем только базовые уровни, остальные создаются при запросе"""
        preset_levels = []
        ...
        # Добавляем только первые 2 уровня для быстрой инициализации
        # Остальные будут генерироваться лениво по запросу
        preset_levels.extend([level1, level2])
        
        # Заполнить заполнители для 8 остальных уровней (будут генерироваться при доступе)
        for i in range(3, 11):
            # Создаем пустой уровень-заполнитель
            preset_levels.append({
                "name": f"Preset {i}",
                "player_start_x": 150,
                "player_start_y": 400,
                "speed": 5,
                ...
                "amethysts": [],
                "random_generation": False,
                "_lazy_generated": False,  # Флаг для ленивой генерации
                "_level_num": i
            })
    ...
    def handle_collisions(self):
        """Оптимизированная обработка коллизий с кэшированием результатов"""
        if not self.player:
            return
            
        player_rect = self.player.get_rect()
        if not player_rect:  # ✅ Добавлена проверка на None
            return
        
        # 🚀 ОПТИМИЗАЦИЯ: Используем пространственное разделение
        # Проверяем только объекты, которые находятся рядом с игроком
        screen_left = self.camera_x - 100
        screen_right = self.camera_x + self.SCREEN_WIDTH + 100
        
        # Список объектов для проверки коллизий (в пределах видимой области + буфер)
        nearby_obstacles = [obj for obj in self.obstacles 
                          if obj.x + obj.w >= screen_left and obj.x <= screen_right]
        
        for obj in nearby_obstacles:
            # Получаем строковое значение типа для сравнения (кэшируем)
            ...
            # 🚀 ОПТИМИЗАЦИЯ: Быстрая AABB коллизия перед точной проверкой
            obj_rect = obj.get_rect()
            if obj_rect and player_rect.colliderect(obj_rect):
                # Определяем тип коллизии и обрабатываем
                ...
                elif obj_type_str == ObstacleType.PORTAL.value:
                    self.handle_portal_collision(obj)
    ...
    def handle_portal_collision(self, portal):
        """Обработка столкновения с порталом завершения уровня"""
    ...
    def safe_load_skin(self):
        try:
            ...
            # Пробуем разные пути
            paths_to_try = [
                f"assets/game_project/skins/{filename}",
                f"assets/skins/processed/{filename}",
    ...
    def process_and_load_skin(self, filename, target_size=None):
        ...
        2. Масштабирует до стандартного размера (55x55)
        ...
        try:
            ...
            #  Все скины масштабируются до 55x55
            # Это гарантирует одинаковый размер для всех скинов
            target_size = 55
    ...
    def add_custom_skin(self, skin_id, image_path, name_ru=None, name_en=None, cost=0, 
        ...
        try:
            ...
            if auto_process:
                output_dir = "assets/skins/processed"
                ...
                if os.path.exists(output_path):
                    ...
                    if processed_image:
                        ...
                        backup_dir = "assets/skins/original"
                ...
                target_path = f"assets/skins/{image_filename}"
    ...
    def load_player_skins(self):
        """ОПТИМИЗАЦИЯ: Кэшируем данные о скинах"""
        if not self.account_system.current_account:
            return
        
        # Проверяем кэш скинов в памяти
        if not hasattr(self, '_cached_skins') or self._cached_skins is None:
            try:
                cursor = self.account_system.conn.cursor()
                cursor.execute('SELECT skin_id FROM player_skins WHERE player_id = ? AND owned = 1', 
                             (self.account_system.current_account["id"],))
                self._cached_skins = [row[0] for row in cursor.fetchall()]
            except sqlite3.Error as e:
                logging.error(f"Error loading player skins: {e}")
                self._cached_skins = []
                return
        
        owned_skins = self._cached_skins
    ...
    def load_player_upgrades(self):
        """ОПТИМИЗАЦИЯ: Быстрое применение кэшированных улучшений с таймаутом"""
        if not self.account_system.current_account:
            return
        
        # Проверяем кэш улучшений в памяти
        if not hasattr(self, '_cached_upgrades') or self._cached_upgrades is None:
            try:
                logging.debug("Loading player upgrades from database...")
                
                # Устанавливаем таймаут для запроса
                self.account_system.conn.timeout = 5.0
                cursor = self.account_system.conn.cursor()
                ...
                self._cached_upgrades = [row[0] for row in results]
                logging.debug(f"Loaded {len(self._cached_upgrades)} upgrades")
                
            except sqlite3.OperationalError as e:
                logging.warning(f"Database timeout loading upgrades, using empty list: {e}")
                self._cached_upgrades = []
                return
            except sqlite3.Error as e:
        ...
        owned_upgrades = self._cached_upgrades
    ...
    def load_resources(self):
        try:
            ...
            sound_paths = [
                "assets/sounds/crash.wav", "assets/game_project/sounds/crash.wav",
                "assets/sounds/collect.wav", "assets/game_project/sounds/collect.wav",
                "assets/sounds/chest_open.wav", "assets/sounds/open chest.wav", "assets/game_project/sounds/open chest.wav",
                "sounds/level_complete.wav",
                "assets/sounds/powerup.wav", "assets/sounds/SFX_Powerup_01.wav", "assets/sounds/SFX_Powerup_02.wav", "assets/game_project/sounds/powerup.wav",
            ...
            image_paths = {
                'menu_bg': ["assets/menu/background.png", "assets/game_project/menu/background.png"],
                'amethyst_image': ["assets/skins/amethyst.png", "assets/game_project/skins/amethyst.png"],
                'chest_image': ["assets/chestskins/amethystchest.png", "assets/game_project/chestskins/amethystchest.png"]
            }
            ...
            try:
                if os.path.exists("assets/backgrounds/game_bg.png"):
                    self.game_bg = pygame.image.load("assets/backgrounds/game_bg.png").convert()
                    self.game_bg = pygame.transform.scale(self.game_bg, (self.SCREEN_WIDTH, self.SCREEN_HEIGHT))
            except Exception as e:
                pass
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                # F1 - показать отладку
                if event.key == pygame.K_F1 and self.state == GameState.PLAYING:
                    if not self.f1_pressed:
                        self.show_debug_info = not self.show_debug_info
                        self.f1_pressed = True
                
                # TAB - показать статистику (только в режиме игры)
                ...
                # 1-7 - переключение тем
                elif pygame.K_1 <= event.key <= pygame.K_7 and self.state == GameState.PLAYING:
                    theme_num = event.key - pygame.K_1
                    if theme_num < len(ThemeType):
                        theme_names = [t for t in ThemeType]
                        self.current_theme = theme_names[theme_num]
                        self.theme_manager.apply_theme(self.current_theme)
                
                # S - использовать способность (shield/защита)
                elif event.key == pygame.K_s and self.state == GameState.PLAYING:
                    if not self.s_key_pressed and hasattr(self, 'analytics'):
                        # Записать использование способности
                        self.s_key_pressed = True
                
                # D - использовать способность (double jump/двойной прыжок)
                elif event.key == pygame.K_d and self.state == GameState.PLAYING:
                    if not self.d_key_pressed and hasattr(self, 'analytics'):
                        # Записать использование способности
                        self.d_key_pressed = True
            
            elif event.type == pygame.KEYUP:
                # Сбросить флаги когда клавиша отпущена
    ...
    def handle_escape_key(self):
        # если мы уже показываем диалог, ESC просто вернёт на главное меню
        if self.state == GameState.MENU and self.menu == getattr(self, 'exit_confirm_menu', None):
            self.return_to_main_menu()
            return

        if self.state == GameState.MENU:
            # если находимся в главном меню или любом другом - показываем подтверждение
            if self.menu == self.main_menu:
                self.show_exit_confirmation()
            else:
                self.return_to_main_menu()
        elif self.state in [GameState.PLAYING, GameState.PAUSED,
                           GameState.SHOP, GameState.INVENTORY, GameState.LEVEL_SELECT,
                           ...
                           GameState.GAME_OVER, GameState.EDITOR]:
            self.return_to_main_menu()

    def show_exit_confirmation(self):
        """Переключиться на меню подтверждения выхода"""
        self.state = GameState.MENU
        self.menu = self.exit_confirm_menu
    ...
    def start_selected_level(self, index=None):
        ...
        logging.info(f"Starting selected level: {selected_level.get('name', 'Unknown')}")
    ...
    def load_custom_level(self, filename):
        try:
            logging.info(f"Loading custom level: {filename}")
            if os.path.exists(filename):
                ...
                if not self._validate_level_data(level_data):
                    logging.warning(f"Invalid level structure in {filename}, using defaults")
                    self.create_default_level(filename)
                    return
                
                logging.info(f"Loaded custom level with {len(level_data.get('obstacles', []))} obstacles")
                self.start_game_with_level(level_data)
            else:
                logging.warning(f"Custom level file not found: {filename}")
                self.create_default_level(filename)
                
        except json.JSONDecodeError as e:
            logging.error(f"Invalid JSON in level {filename}: {e}")
            self.create_default_level(filename)
        except Exception as e:
            logging.error(f"Error loading level {filename}: {e}")
            self.create_default_level(filename)
    
    def _validate_level_data(self, level_data: dict) -> bool:
        """✅ Валидирует структуру данных уровня"""
        try:
            required_fields = ["obstacles", "amethysts"]
            for field in required_fields:
                if field not in level_data:
                    logging.warning(f"Missing required field: {field}")
                    return False
            
            if not isinstance(level_data.get("obstacles"), list):
                logging.warning("obstacles must be a list")
                return False
            
            if not isinstance(level_data.get("amethysts"), list):
                logging.warning("amethysts must be a list")
                return False
            
            # Валидация препятствий
            for obs in level_data.get("obstacles", []):
                if not all(k in obs for k in ["x", "y", "type"]):
                    logging.warning("Obstacle missing required fields")
                    return False
            
            return True
        except Exception as e:
            logging.error(f"Error validating level data: {e}")
            return False
    ...
    def generate_chest_rewards(self):
        ...
        try:
            # Основной пул шансов (тип, amount, weight)
            reward_chances = [
                ("amethyst", 10, 50),
                ("amethyst", 25, 30),
                ("amethyst", 50, 15),
            ...
            main_reward = random.choices(reward_chances, weights=[c[2] for c in reward_chances])[0]
            ...
            if reward_type == "amethyst":
                ...
                if self.account_system.conn:
                    try:
                        ...
                        logging.error(f"Database error updating chest amethysts: {e}")

            elif reward_type == "skin":
                locked_skins = [skin_id for skin_id, skin in self.skins.items() 
                              if not skin.get("owned") and not skin.get("locked")]

                if locked_skins:
                    skin_id = random.choice(locked_skins)
                    self.skins[skin_id]["owned"] = True
                    skin_name = self.skins[skin_id].get("name", skin_id)
                    if isinstance(skin_name, dict):
                        skin_name = skin_name.get(self.language, skin_name.get('ru', 'Unknown'))
        ...
        # Подготавливаем данные для анимации "спина" — список визуальных элементов
        display_items = []
        for r in rewards:
            if r["type"] == "amethyst":
                display_items.append({"label": f'+{r["amount"]} AM', "type": "amethyst", "data": r})
            elif r["type"] == "skin":
                display_items.append({"label": r.get("name", "Skin"), "type": "skin", "data": r})
            else:
                display_items.append({"label": r.get("text", "Message"), "type": r.get("type"), "data": r})

        # Заполняем спин-окно фейковыми предметами, чтобы было 8-12 элементов
        fillers = [
            {"label": "+5 AM", "type": "amethyst", "data": {"amount": 5}},
            {"label": "+10 AM", "type": "amethyst", "data": {"amount": 10}},
            {"label": "+25 AM", "type": "amethyst", "data": {"amount": 25}},
            {"label": "Rare Skin", "type": "skin", "data": {}},
            {"label": "+50 AM", "type": "amethyst", "data": {"amount": 50}},
        ]

        self.chest_spin_items = (display_items + fillers)[:]
        random.shuffle(self.chest_spin_items)
        # Убедимся, что реальный приз присутствует в списке
        for it in display_items:
            if it not in self.chest_spin_items:
                self.chest_spin_items.append(it)

        self.chest_rewards = rewards
        self.chest_spin_index = 0
        self.chest_spin_running = True
        self.chest_spin_timer = 0
        self.chest_spin_duration = 120  # количество фреймов спина до остановки
        self.chest_selected_index = None

    def start_game(self):
        logging.info("Starting random game")
        # Меню уже должно быть отключено из start_game_from_menu(), но добавим проверку
        # Не пытаемся отключить меню снова если оно уже отключено
    ...
    def start_game_with_level(self, level_data):
        """ОПТИМИЗАЦИЯ: Быстрая инициализация уровня"""
        logging.info(f"Starting game with level: {level_data.get('name', 'Unknown')}")
        self.state = GameState.PLAYING
        logging.debug("State set to PLAYING")
        
        # Воспроизводим звук начала игры (асинхронно)
        if self.start_game_sound:
            logging.debug("Playing start game sound")
            self.play_sound_safe(self.start_game_sound)
        
        # Инициализируем базовые параметры
        logging.debug("Initializing game parameters")
        self.score = 0
        ...
        logging.debug(f"Scale: x={scale_x:.2f}, y={scale_y:.2f}")
        
        # Масштабируем конец уровня
        ...
        # Создаем игрока с масштабированной позицией
        ...
        logging.debug(f"Player created at ({self.player.x}, {self.player.y})")
        
        # ОПТИМИЗАЦИЯ: Кэшируем улучшения при загрузке профиля
        if hasattr(self, 'account_system') and self.account_system.current_account:
            logging.debug("Loading player upgrades")
            self.load_player_upgrades()
            logging.debug(f"Upgrades loaded: {self.player_upgrades}")

        # ОПТИМИЗАЦИЯ: Быстрое создание препятствий
        obstacles_data = level_data.get("obstacles", [])
        logging.debug(f"Creating {len(obstacles_data)} obstacles")
        self.obstacles = []
        ...
        for i, obs_data in enumerate(obstacles_data):
            ...
            # Инициализируем дополнительные параметры если нужно
        ...
        logging.debug(f"Obstacles created: {len(self.obstacles)}")

        # ОПТИМИЗАЦИЯ: Быстрое создание аметистов
        amethysts_data = level_data.get("amethysts", [])
        logging.debug(f"Creating {len(amethysts_data)} amethysts")
        self.amethysts = []
        
        min_scale = min(scale_x, scale_y)
        for amethyst_data in amethysts_data:
            scaled_x = int(amethyst_data["x"] * scale_x)
            scaled_y = int(amethyst_data["y"] * scale_y)
            scaled_size = int(amethyst_data.get("size", 30) * min_scale)
            self.amethysts.append(Amethyst(
                scaled_x,
                scaled_y,
                scaled_size
        ...
        logging.debug(f"Amethysts created: {len(self.amethysts)}")
        
        # Создаем портал если это пресет-уровень
        if not self.enable_random_generation:
            logging.debug("Creating portal for preset level")
            portal_x = int(self.level_end_x - 150)
            portal_y = self.SCREEN_HEIGHT - 200
            portal_w = 150
            portal_h = 200
            portal = Obstacle(
                portal_x,
            ...
            self.obstacles.append(portal)
        
        # Инициализируем бонусы
        logging.debug("Initializing bonuses")
        self.bonuses = []
        self.magnet_active = False
        self.magnet_timer = 0
        self.slow_time_active = False
        ...
        logging.info(f"Game initialized successfully! Total obstacles: {len(self.obstacles)}, Amethysts: {len(self.amethysts)}")
    ...
    def buy_shop_item(self):
        """ОПТИМИЗАЦИЯ: Минимизируем операции БД"""
        if not self.account_system.current_account:
        ...
        # Проверяем кэш улучшений
        if hasattr(self, '_cached_upgrades') and self._cached_upgrades and effect_type_str in self._cached_upgrades:
        ...
        try:
            ...
            # Обновляем кэш
            if self._cached_upgrades is not None:
                self._cached_upgrades.append(effect_type_str)
    ...
    def update_game(self):
        ...
        # 📊 ОТСЛЕЖИВАНИЕ АНАЛИТИКИ - Обновление производительности
        # (record_frame_time вызывается в основном цикле с реальным временем кадра)
        
        # 📊 ОТСЛЕЖИВАНИЕ АНАЛИТИКИ - Проверка изменения комбо
        current_combo = self.combo if hasattr(self, 'combo') else 0
        if current_combo > self.last_combo:
            self.analytics.record_combo(current_combo)
            self.last_combo = current_combo
        
        # 📊 ОТСЛЕЖИВАНИЕ АНАЛИТИКИ - Проверка прыжков
        current_jumps = self.stats.get("total_jumps", 0)
        if current_jumps > self.last_jump_count:
            jumps_in_this_frame = current_jumps - self.last_jump_count
            for _ in range(jumps_in_this_frame):
                self.analytics.record_jump(height=0, distance=0)
            self.last_jump_count = current_jumps
        
        # 📊 ОТСЛЕЖИВАНИЕ АНАЛИТИКИ - Отслеживание сбора предметов
        current_amethysts = self.total_amethysts
        if hasattr(self, '_last_amethysts_count'):
            if current_amethysts > self._last_amethysts_count:
                items_collected = current_amethysts - self._last_amethysts_count
                for _ in range(items_collected):
                    self.analytics.record_item_collected("amethyst", 1)
                self._last_amethysts_count = current_amethysts
        else:
            self._last_amethysts_count = current_amethysts
        ...
        if self.player.y >= self.ground_y - self.ground_buffer:
        ...
        if self.player.y > self.death_y:
            # 📊 ОТСЛЕЖИВАНИЕ АНАЛИТИКИ - Запись смерти
            if self.level_start_time:
                level_duration = time.time() - self.level_start_time
                self.analytics.record_death(reason="fall", level=self.current_level, distance=self.player.x)
            else:
                self.analytics.record_death(reason="fall", level=self.current_level, distance=self.player.x)
        ...
        if not self.enable_random_generation:
            # Используем позицию игрока как основу для прогресса
            self.distance_traveled = self.player.x
            self.percent = self.calculate_level_progress()
            
            # ДОБАВЛЕНИЕ: Автоматическое завершение при достижении конца уровня
            if self.player.x >= self.level_end_x and not self.level_complete:
                self.level_complete = True
                self.play_sound_safe(self.level_complete_sound or self.chest_sound)
                self.particle_system.add_particles(
                    self.player.x + self.player.size//2,
                ...
                # 📊 ОТСЛЕЖИВАНИЕ АНАЛИТИКИ - Запись завершения уровня
                if self.level_start_time:
                    level_duration = time.time() - self.level_start_time
                    self.analytics.record_level_complete(level_name=self.current_level, time_taken=level_duration, score=self.score)
                else:
                    self.analytics.record_level_complete(level_name=self.current_level, time_taken=0, score=self.score)
            ...
            # ДОБАВЛЕНИЕ: Дополнительная проверка - если игрок прошел конец уровня, завершить (только для preset уровней)
            if not self.enable_random_generation and self.player.x >= self.level_end_x and not self.level_complete:
                self.level_complete = True
                self.play_sound_safe(self.level_complete_sound or self.chest_sound)
                self.particle_system.add_particles(
                    self.player.x + self.player.size//2,
        ...
        if self.enable_random_generation:
            ...
            if self.enable_random_generation:
                self.percent = int(self.distance_traveled / 15)  # Не ограничиваем для бесконечной генерации
        ...
        # ✅ Более эффективная очистка: удаляем только старые элементы, а не пересоздаем весь список
        if len(self.obstacles) > 150:
            del self.obstacles[:len(self.obstacles) - 100]
        ...
        if len(self.amethysts) > 80:
            del self.amethysts[:len(self.amethysts) - 50]
        ...
        if hasattr(self, 'bonuses'):
            ...
            # ✅ Более эффективная очистка
            if len(self.bonuses) > 40:
                del self.bonuses[:len(self.bonuses) - 20]
    ...
    def spawn_random_obstacle(self):
        # Добавляем больше разнообразия и логируем спавн для отладки
        try:
            obstacle_type = random.choices(
                [ObstacleType.SPIKE, ObstacleType.PLATFORM, ObstacleType.MOVING_SPIKE, ObstacleType.SPIKE_CLUSTER, ObstacleType.BOUNCING_PLATFORM, ObstacleType.DISAPPEARING_PLATFORM, ObstacleType.ICE_PLATFORM],
                weights=[0.30, 0.25, 0.15, 0.08, 0.08, 0.07, 0.07]
            )[0]
            ...
            # Логируем спавн для отладки
            logging.info(f"Spawned obstacle: {obstacle_type} — total now {len(self.obstacles)}")

        except Exception as e:
            logging.error(f"Error in spawn_random_obstacle: {e}")
    # РЕДАКТОР УРОВНЕЙ
    ...
    def handle_editor_events(self, event):
        if not hasattr(self, "editor_panel_y"):
            self.editor_panel_y = self.SCREEN_HEIGHT - 120
            self.editor_panel_height = 120
    ...
    def draw_editor(self):
        ...
        for obj in self.editor_obstacles:
            ...
            if -100 < x < self.SCREEN_WIDTH + 100 and -100 < y < self.SCREEN_HEIGHT + 100:
                ...
                if obj_type_str == ObstacleType.SPIKE.value:
                    ...
                    # Отрисовка ледяной платформы - прозрачная
                    ice_surf = pygame.Surface((obj.w, obj.h), pygame.SRCALPHA)
                    pygame.draw.rect(ice_surf, (173, 216, 230, 100), (0, 0, obj.w, obj.h))
                    self.screen.blit(ice_surf, (x, y))
                    color = (135, 206, 250) if obj == self.editor_selected_object else (100, 180, 220)
                    pygame.draw.rect(self.screen, color, (x, y, obj.w, obj.h), 3)
                
                elif obj_type_str == ObstacleType.ROTATING_SPIKE.value:
                    points = [(x+20, y+60), (x+40, y), (x+60, y+60)]
                    color = (255, 100, 100) if obj == self.editor_selected_object else (200, 80, 80)
                    pygame.draw.polygon(self.screen, color, points)
                    pygame.draw.polygon(self.screen, self.WHITE, points, 2)
                
                elif obj_type_str == ObstacleType.BOUNCING_PLATFORM.value:
                    rect = (x, y, obj.w, obj.h)
                    color = (0, 255, 100) if obj == self.editor_selected_object else (0, 200, 80)
                    pygame.draw.rect(self.screen, color, rect)
                    pygame.draw.rect(self.screen, self.WHITE, rect, 3)
                
                elif obj_type_str == ObstacleType.DISAPPEARING_PLATFORM.value:
                    rect = (x, y, obj.w, obj.h)
                    color = (255, 165, 0) if obj == self.editor_selected_object else (200, 130, 0)
                    pygame.draw.rect(self.screen, color, rect, 2)
                    pygame.draw.line(self.screen, color, (x, y), (x + obj.w, y + obj.h), 1)
                    pygame.draw.line(self.screen, color, (x + obj.w, y), (x, y + obj.h), 1)
                
                elif obj_type_str == ObstacleType.PORTAL.value:
                    # Отрисовка портала завершения уровня
                    time_ms = pygame.time.get_ticks()
                    pulse = math.sin(time_ms * 0.01) * 5 + math.sin(time_ms * 0.005) * 3
                    portal_width = obj.w + pulse
                    portal_rect = pygame.Rect(x - pulse/2, y, portal_width, obj.h)
                    color = (0, 255, 255) if obj == self.editor_selected_object else (0, 200, 200)
                    pygame.draw.rect(self.screen, color, portal_rect)
                    pygame.draw.rect(self.screen, self.WHITE, portal_rect, 3)
                    # Энергетические эффекты
                    for i in range(5):
                        effect_x = x + random.randint(0, obj.w)
                        effect_y = y + random.randint(0, obj.h)
                        pygame.draw.circle(self.screen, (255, 255, 255), (effect_x, effect_y), 2)
        ...
        # Отображение информации редактора
        ...
        obstacle_types = [
            ...
            (ObstacleType.PORTAL, "Портал", (0, 255, 255)),
        ...
        for i, (obs_type, name, color) in enumerate(obstacle_types):
            ...
            elif obs_type == ObstacleType.PORTAL:
                # Портал
                pygame.draw.rect(self.screen, color, (x_pos + 5, icon_y + 5, panel_item_width - 10, 20))
                pygame.draw.circle(self.screen, self.WHITE, (x_pos + panel_item_width // 2, icon_y + 15), 3)
            else:
    ...
    def save_level(self):
        try:
            ...
            # Расчет level_end_x на основе объектов
            max_obj_x = 0
            if obstacles_data:
                max_obj_x = max(o["x"] + o.get("w", 60) for o in obstacles_data)
            
            max_ame_x = 0
            if amethysts_data:
                max_ame_x = max(a["x"] + 50 for a in amethysts_data)
            
            calculated_end_x = max(max_obj_x + 500, max_ame_x + 500, 2000)
    ...
    def load_level(self):
        try:
            ...
            # ДОБАВЛЕНИЕ: Добавляем портал в редактор для preset уровней
            if self.editor_level_name != "my_level":
                portal_x = int(self.level_end_x - 150)  # Портал чуть перед концом уровня
                portal_y = self.SCREEN_HEIGHT - 200  # Яма внизу
                portal_w = 150  # Шире для лучшей видимости
                portal_h = 200
                portal = Obstacle(
                    portal_x,
                    portal_y,
                    ...
                    portal_w,
                    portal_h,
                    (0, 255, 255)  # Голубой цвет для портала
                )
                self.editor_obstacles.append(portal)
    ...
    def draw_game(self):
        time_ms = pygame.time.get_ticks()
        # Кэшировать фон каждые 50 мс для оптимизации
        if not hasattr(self, 'cached_dynamic_bg_time') or time_ms - self.cached_dynamic_bg_time > 50:
            self.cached_dynamic_bg = self.bg_generator.create_dynamic_background(
                time_ms=time_ms,
                bg_color1=(20, 40, 80),
                bg_color2=(50, 100, 150),
                accent_color=(100, 180, 255)
            )
            self.cached_dynamic_bg_time = time_ms
        self.screen.blit(self.cached_dynamic_bg, (0, 0))
        ...
        if not self.enable_random_generation:
            ...
            if -100 < portal_x < self.SCREEN_WIDTH + 100:
                ...
                # Упрощенный финиш портал (без дорогих 3-слойных волн)
        ...
        for obj in self.obstacles:
            ...
            if -100 < x < self.SCREEN_WIDTH + 100:
                ...
                if obj_type_str == ObstacleType.SPIKE.value:
                    ...
                    if obj.visible:
                        # Просто контур платформы (без дорогого Surface creation)
                ...
                # КОНТЕНТ: Отрисовка ледяной платформы
                    ...
                    # Ледяной эффект (оптимизировано)
                    # Просто полупрозрачный прямоугольник без дорогих Surface
                    pygame.draw.rect(self.screen, (173, 216, 230), (x, obj.y, obj.w, obj.h))
                    pygame.draw.rect(self.screen, (135, 206, 250), (x, obj.y, obj.w, obj.h), 2)
                
                # ДОБАВЛЕНИЕ: Отрисовка портала завершения уровня
                elif obj_type_str == ObstacleType.PORTAL.value:
                    # Портал как яма (оптимизировано)
                    portal_rect = pygame.Rect(x, obj.y, obj.w, obj.h)
                    pygame.draw.rect(self.screen, (0, 0, 0), portal_rect)  # Чёрная яма
                    
                    # Пульсирующая граница портала (вместо дорогих layers)
                    time_ms = pygame.time.get_ticks()
                    pulse = math.sin(time_ms * 0.005) * 2
                    border_width = int(4 + pulse)
                    pygame.draw.rect(self.screen, obj.color, portal_rect, max(2, border_width))
        ...
        if self.player:
            ...
            # Эффект щита (оптимизировано)
            if self.player.invincible:
                shield_radius = self.player.size + 5
                pygame.draw.circle(self.screen, (0, 255, 255), 
                                 (int(px + self.player.size//2), int(py + self.player.size//2)), 
                                 int(shield_radius), 2)
            
            # Эффекты даша (удалены дорогие trail эффекты)
            
            # Энергетические волны вокруг игрока (упрощено)
            if self.is_dashing:
                # Просто простой круг вместо дорогого Surface эффекта
                wave_radius = self.player.size + 10
                pygame.draw.circle(self.screen, (0, 255, 255), 
                                 (int(px + self.player.size//2), int(py + self.player.size//2)), 
                                 int(wave_radius), 2)
                
            
            # Отображение игрока с анимацией
            if self.player_image:
                # Используем анимацию вместо статичного изображения
                current_frame = self.animator.get_current_frame()
                if current_frame:
                    scaled_frame = pygame.transform.scale(current_frame, (self.player.size, self.player.size))
                    self.screen.blit(scaled_frame, (px, py))
                else:
                    self.screen.blit(self.player_image, (px, py))
            else:
                # Используем анимацию для базового игрока
                current_frame = self.animator.get_current_frame()
                if current_frame:
                    scaled_frame = pygame.transform.scale(current_frame, (self.player.size, self.player.size))
                    self.screen.blit(scaled_frame, (px, py))
        ...
        # UI ЭЛЕМЕНТЫ: Кнопка паузы и прогресс-бары
        # UI элементы удалены - используется только pygame-menu
        ...
        # ⚡ ОТРИСОВКА ЛИНИЙ СКОРОСТИ
        active_lines = []
        for lines in self.speed_lines_list:
            if lines.update():
                lines.draw(self.screen, self.camera_x, velocity=abs(self.player.velocity_y) if self.player else 0)
                active_lines.append(lines)
        self.speed_lines_list = active_lines
    ...
    def draw_level_complete(self):
        ...
        # 🎨 ОТРИСОВКА УЛУЧШЕНИЙ - Применение темы и статистики
        # Применить цвета текущей темы (если применима)
        if hasattr(self, 'theme_manager') and hasattr(self, 'current_theme'):
            current_theme = self.theme_manager.themes.get(self.current_theme)
            if current_theme:
                # Применить цвет фона (для UI элементов) - опционально
                # Обновить основные цвета для текстур/фонов
                pass
        
        # Показать экран статистики если нажата TAB
        if hasattr(self, 'show_stats_screen') and self.show_stats_screen and hasattr(self, 'stats_screen'):
            self.stats_screen.draw(self.screen)
        
        # Показать отладочную информацию если нажата F1
        if hasattr(self, 'show_debug_info') and self.show_debug_info:
            if hasattr(self, 'performance_monitor'):
                debug_info = self.performance_monitor.get_debug_info()
                debug_y = 10
                for line in debug_info[:10]:  # Показать максимум 10 строк
                    debug_surf = self.tiny_font.render(line, True, self.CYAN)
                    self.screen.blit(debug_surf, (10, debug_y))
                    debug_y += 18
        
        # Подсказка в углу экрана
        hint = "TAB: Stats | F1: Debug" if self.language == 'en' else "TAB: Статистика | F1: Отладка"
        hint_surf = self.tiny_font.render(hint, True, (100, 100, 100))
        self.screen.blit(hint_surf, (self.SCREEN_WIDTH - hint_surf.get_width() - 10, self.SCREEN_HEIGHT - 20))
    ...
    def draw_chest(self):
        ...
        # Рисуем сундук статично внизу центра
        chest_x = self.SCREEN_WIDTH // 2 - 100
        chest_y = 140
        chest_rect = pygame.Rect(chest_x, chest_y, 200, 160)
        pygame.draw.rect(self.screen, (120, 70, 20), chest_rect, border_radius=8)
        pygame.draw.rect(self.screen, (200, 150, 100), (chest_x, chest_y, 200, 50), 4, border_radius=6)

        # Спин-окно: показываем текущий элемент в центре
        window_w, window_h = 420, 120
        window_x = self.SCREEN_WIDTH // 2 - window_w // 2
        window_y = chest_y + 200
        pygame.draw.rect(self.screen, (30, 30, 40), (window_x, window_y, window_w, window_h), border_radius=6)
        pygame.draw.rect(self.screen, self.CYAN, (window_x, window_y, window_w, window_h), 3, border_radius=6)

        # Инициализация данных для анимации, если нужно
        if not hasattr(self, 'chest_spin_items') or not self.chest_spin_items:
            # Если вдруг не подготовлено — показываем простую информацию
            for i, r in enumerate(self.chest_rewards):
                text = self.font.render(str(r.get('amount', r.get('name', r.get('text', '')))), True, self.WHITE)
                self.screen.blit(text, (self.SCREEN_WIDTH // 2 - text.get_width() // 2, window_y + 10 + i * 28))
            hint_text = self.small_font.render(self.localization.get('click_to_continue'), True, self.WHITE) if hasattr(self.localization, 'get') else self.small_font.render('Click to continue', True, self.WHITE)
            self.screen.blit(hint_text, (self.SCREEN_WIDTH // 2 - hint_text.get_width() // 2, self.SCREEN_HEIGHT - 50))
            return

        # Спин логика
        if getattr(self, 'chest_spin_running', False):
            # Увеличиваем таймер и индекс спина
            self.chest_spin_timer += 1
            # Скорость прокрутки зависит от таймера — замедляем перед остановкой
            speed = 6 if self.chest_spin_timer < self.chest_spin_duration * 0.6 else 2
            if self.chest_spin_timer % speed == 0:
                self.chest_spin_index = (self.chest_spin_index + 1) % len(self.chest_spin_items)

            # Отрисовываем несколько элементов слева и справа для эффекта полосы
            center_i = self.chest_spin_index
            for offset, slot in enumerate(range(-3, 4)):
                idx = (center_i + slot) % len(self.chest_spin_items)
                item = self.chest_spin_items[idx]
                # позиционируем по полосе
                pos_x = self.SCREEN_WIDTH // 2 + slot * 90
                pos_y = window_y + window_h // 2
                alpha = max(40, 255 - abs(slot) * 60)
                surf = self.font.render(item['label'], True, self.WHITE)
                surf.set_alpha(alpha)
                rect = surf.get_rect(center=(pos_x, pos_y))
                self.screen.blit(surf, rect)

            # Останавливаем спин после длительности
            if self.chest_spin_timer >= self.chest_spin_duration:
                # Выбираем текущий индекс как итог
                self.chest_selected_index = self.chest_spin_index % len(self.chest_spin_items)
                self.chest_spin_running = False
                # Создаём падение выбранного предмета
                sel = self.chest_spin_items[self.chest_selected_index]
                self.chest_drops = []
                # одна падающая сущность
                self.chest_drops.append({
                ...
                # Сыграть звук открытия
                self.play_sound_safe(getattr(self, 'chest_sound', None))
        else:
            # Если спин уже остановлен — анимируем падение и отображаем итог
            if hasattr(self, 'chest_drops') and self.chest_drops:
                for drop in self.chest_drops:
                    if drop['alive']:
                        drop['x'] += drop['vx']
                        drop['vy'] += 0.2
                        drop['y'] += drop['vy']
                        ...
                        txt = self.font.render(label, True, self.GOLD if drop['item']['type']=='skin' else self.PURPLE)
                        rotated = pygame.transform.rotate(txt, drop['rot'])
                        rrect = rotated.get_rect(center=(int(drop['x']), int(drop['y'])))
                        self.screen.blit(rotated, rrect)
                        # частицы
                        for _ in range(2):
                            px = rrect.centerx + random.randint(-10, 10)
                            py = rrect.centery + random.randint(-10, 10)
                            pygame.draw.circle(self.screen, (255,255,255), (px, py), 2)
                        # Если дошло до пола — фиксируем
                        if drop['y'] > self.SCREEN_HEIGHT - 150:
                            drop['alive'] = False
            else:
                # Отобразить итогный элемент в окне
                sel = self.chest_spin_items[self.chest_selected_index]
                val = sel['label']
                txt = self.title_font.render(val, True, self.GOLD if sel['type']=='skin' else self.PURPLE)
                rect = txt.get_rect(center=(self.SCREEN_WIDTH//2, window_y + window_h//2))
    ...
    def draw(self):
        ...
        if self.state == GameState.INTRO:
            self.draw_intro()
        elif self.state == GameState.MENU and self.menu:
        ...
        try:
            while self.running:
                try:
                    ...
                    # Обновление UI и анимаций
                    self.ui_manager.update(pygame.mouse.get_pos(), pygame.mouse.get_pressed()[0])
                    
                    # Обновление анимации игрока в зависимости от состояния
                    if self.player.vy < 0:  # Игрок прыгает вверх
                        self.animator.set_animation('jump')
                    else:  # Игрок падает или стоит
                    ...
                    # 🚀 FPS CAP: Ограничиваем FPS для оптимизации
                    frame_time_ms = self.clock.tick(self.max_fps)
                    self.performance_monitor.record_frame_time(frame_time_ms)
                    
                    # 🔧 ДИНАМИЧЕСКАЯ ОПТИМИЗАЦИЯ: Автоматическое снижение частиц при лагах
                    if self.state == GameState.PLAYING:
                        avg_frame_time = self.performance_monitor.get_average_frame_time()
                        # Если среднее время кадра > 20мс (FPS < 50), снижаем частицы
                        if avg_frame_time > 20:
                            new_limit = max(self.min_particles_limit, self.max_particles_limit - 25)
                            if new_limit < self.max_particles_limit:
                                self.max_particles_limit = new_limit
                                self.particle_system.max_particles = new_limit
                        # Если всё хорошо (< 15мс), постепенно увеличиваем обратно
                        elif avg_frame_time < 15 and self.max_particles_limit < 250:
                            self.max_particles_limit = min(250, self.max_particles_limit + 10)
                            self.particle_system.max_particles = self.max_particles_limit
                    
                    # Проверяем использование памяти кэша текстур
                    if self.state == GameState.PLAYING:
                        cache_stats = self.texture_cache.get_stats()
                        if cache_stats['size'] > cache_stats['max_size'] * 0.8:
                            self.texture_cache.reduce_memory()
                    
                except Exception as e:
                    logging.error(f"Error in main game loop: {e}")
                    # Продолжаем выполнение, чтобы игра не крашилась
    ...
    def save_on_exit(self):
        ...
        # 📊 ОЧИСТКА УЛУЧШЕННЫХ СИСТЕМ - Сохранение данных при выходе
        if hasattr(self, 'analytics'):
            try:
                playtime = time.time() - self.session_start_time if hasattr(self, 'session_start_time') else 0
                self.analytics.end_session(playtime)
            except Exception as e:
